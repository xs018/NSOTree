...
from models.nostree import CoxNOSTree, CoxCCNOSTree, CoxTimeNOSTree
...
if __name__ == "__main__":
    ...
    if 'simulated' in dataset:
        ...
        labtrans = CoxTimeNOSTree.label_transform()
        ...
        val = tt.tuplefy(X_valid, y_valid)
            
        input_dim = X_train.shape[1]
        output_dim = 1
    
        model = CoxTimeNOSTree(labtrans=labtrans,
                                input_dim=input_dim, 
                                output_dim=output_dim, 
                                hidden_dim=hidden_dim, 
                                num_layer=depth, 
        ...
        log = model.fit(X_train, y_train, batch_size, epochs, callbacks, verbose,
                val_data=val.repeat(10).cat(), val_batch_size=batch_size)
